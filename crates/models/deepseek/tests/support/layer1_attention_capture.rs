//! Test-only native layer-one attention against its source capture.
//!
//! Layer one owns the ratio-two compressed publication. This helper cross-checks
//! its native owner output and uses the production layer-attention adapter.
...
const FIXTURE_SHA256: &str = "a13bb6cd53406f04e436f119aa8dec2184ca43a4d4f4969205bff8bdf26ac31b";
...
fn fixture() -> Value {
    let raw = include_str!("../../../../../fixtures/deepseek-v41/layer1-attention-reference.json");
    assert_eq!(
        format!("{:x}", Sha256::digest(raw.as_bytes())),
        FIXTURE_SHA256
    );
    let fixture: Value = serde_json::from_str(raw).expect("layer-one attention fixture JSON");
...
fn weights(root: &Value) -> Weights {
    ...
    Weights {
        wq_a_codes: fp8(tensor("layers.1.attn.wq_a.weight")),
        wq_a_scales: fp8(tensor("layers.1.attn.wq_a.scale")),
        q_norm: bf16(tensor("layers.1.attn.q_norm.weight")),
        wq_b_codes: fp8(tensor("layers.1.attn.wq_b.weight")),
        wq_b_scales: fp8(tensor("layers.1.attn.wq_b.scale")),
        wkv_codes: fp8(tensor("layers.1.attn.wkv.weight")),
        wkv_scales: fp8(tensor("layers.1.attn.wkv.scale")),
        kv_norm: bf16(tensor("layers.1.attn.kv_norm.weight")),
        attn_sink: fp32(tensor("layers.1.attn.attn_sink")),
        wo_a: bf16(tensor("layers.1.attn.wo_a.weight")),
        wo_b_codes: fp8(tensor("layers.1.attn.wo_b.weight")),
        wo_b_scales: fp8(tensor("layers.1.attn.wo_b.scale")),
    }
}

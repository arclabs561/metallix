...
# ]
# ///
"""Project the source layer-one FFN tail that feeds the layer-two entry."""
...
def _tensor(
    ...
    actual_shape = record.get("shape")
    if (
        record.get("dtype") != dtype
        or actual_shape != shape
        or not isinstance(actual_shape, list)
        or not all(
            type(dimension) is int and dimension >= 0 for dimension in actual_shape
        )
...
def _parameters(encoded: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if any(value is None for value in selected.values()):
        raise RuntimeError("layer-one fixture lacks a routed or shared MoE parameter")
    ...
    expected = {
        f"{LAYER}.hc_attn_fn": ("torch.float32", [8, 256]),
        f"{LAYER}.hc_attn_base": ("torch.float32", [8]),
        f"{LAYER}.hc_attn_scale": ("torch.float32", [3]),
        f"{LAYER}.attn_norm.weight": ("torch.bfloat16", [128]),
...
def layer1_tail_fixture(receipt: dict[str, object]) -> dict[str, object]:
    ...
    if any(
        ...
        raise RuntimeError(
            "layer-one FFN fixture has unexpected reduced model geometry"
        )
    ...
    for step in steps:
        ...
        if set(by_kind) != {"attention", "ffn"}:
            raise RuntimeError("step lacks both layer-one HC observations")
        ...
        for record in (
            ...
            if (
                ...
                raise RuntimeError("layer-one HC coefficient position shape changed")
        block_input = _object(
            intermediates.get("layers.1.block_input"), "layer-one block input"
        )
        residual = _tensor(
            block_input.get("residual"),
            "layer-one input residual",
        ...
        gate = intermediates.get(f"{LAYER}.ffn.gate")
        if not isinstance(gate, list) or len(gate) != 2:
            raise TypeError("layer-one route gate must return weights and indices")
        gate_weights = _tensor(
            gate[0],
            "layer-one gate weights",
            dtype="torch.float32",
            shape=[sequence, 2],
        )
        gate_indices = _tensor(
            gate[1], "layer-one gate indices", dtype="torch.int64", shape=[sequence, 2]
        )
        after_attention = _tensor(
            intermediates.get(f"{LAYER}.after_attention_residual"),
            "layer-one post-attention residual",
            dtype="torch.bfloat16",
            shape=[1, sequence, 2, 128],
        )
        collapsed = _tensor(
            intermediates.get(f"{LAYER}.ffn_collapsed"),
            "layer-one FFN collapsed input",
            dtype="torch.bfloat16",
        ...
        if not isinstance(terminal, list) or len(terminal) != 2:
            raise TypeError(
                "layer-one terminal state must be residual plus next pre-mix"
            )
        output = _tensor(
            terminal[0],
            "layer-one terminal residual",
            dtype="torch.bfloat16",
            shape=[1, sequence, 2, 128],
        )
        next_pre = _tensor(
            terminal[1],
            "layer-one returned pre-mix",
            dtype="torch.float32",
            shape=[1, sequence, 2],
        )
        layer_two = _object(
            intermediates.get("layers.2.block_input"), "layer-two block input"
        )
        layer_two_residual = _tensor(
            layer_two.get("residual"),
            "layer-two incoming residual",
            dtype="torch.bfloat16",
            shape=[1, sequence, 2, 128],
        )
        layer_two_pre = _tensor(
            layer_two.get("incoming_pre"),
            "layer-two incoming pre-mix",
            dtype="torch.float32",
            shape=[1, sequence, 2],
        )
        if output["storage_sha256"] != layer_two_residual["storage_sha256"]:
            raise RuntimeError(
                "layer-one terminal residual does not feed the layer-two residual"
            )
        if next_pre["storage_sha256"] != layer_two_pre["storage_sha256"]:
            raise RuntimeError("layer-one returned pre-mix does not feed layer two")
        raw = {
        ...
        cases.append(
            {
                ...
                "attention_output": _tensor(
                    intermediates.get("layers.1.attn"),
                    "layer-one attention output",
                    dtype="torch.bfloat16",
                    shape=[1, sequence, 128],
                ),
                ...
                "output": output,
                "next_pre": next_pre,
                "layer_two_residual": layer_two_residual,
...
def main() -> int:
    ...
    fixture = layer1_tail_fixture(_runner().run_capture())
    payload = (
        json.dumps(fixture, indent=2, sort_keys=True, allow_nan=False) + "\n"
    ).encode()
    if len(payload) > MAX_FIXTURE_BYTES:
        raise RuntimeError(f"layer-one FFN fixture exceeds {MAX_FIXTURE_BYTES} bytes")
        ...
        json.dumps(
            {
                ...
                "status": "source_layer_one_ffn_fixture",
            },
            sort_keys=True,
        )
    )

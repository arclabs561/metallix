...
# ]
# ///
"""Project a bounded source receipt for the layer-one Engram-to-block seam."""
...
PARAMETER_LAYOUTS = {
    "layers.1.engram.embed.weight": ("torch.float8_e4m3fn", [72, 32]),
    "layers.1.engram.embed.scale": ("torch.float8_e8m0fnu", [72, 1]),
    "layers.1.engram.wkv.weight": ("torch.float8_e4m3fn", [384, 192]),
    "layers.1.engram.wkv.scale": ("torch.float8_e8m0fnu", [12, 6]),
    "layers.1.engram.q_weight": ("torch.bfloat16", [2, 128]),
    "layers.1.engram.k_weight": ("torch.bfloat16", [2, 128]),
}
...
def engram_fixture(receipt: dict[str, object]) -> dict[str, object]:
    """Project only source records needed for native layer-one Engram replay."""
    if (
    ...
    for step in steps:
        ...
        gate_input = _require_dict(
            intermediates.get("layers.1.engram_input"), "layer-one Engram input"
        )
        ...
        embedding = _require_tensor(
            intermediates.get("layers.1.engram.embed"),
            "layer-one Engram embedding",
            dtype="torch.bfloat16",
            shape=[1, sequence, 6, 32],
        )
        wkv = _require_tensor(
            intermediates.get("layers.1.engram.wkv"),
            "layer-one Engram WKV",
            dtype="torch.bfloat16",
        ...
        block_residual = _require_tensor(
            ...
            "layer-one block residual",
            dtype="torch.bfloat16",
            shape=[1, sequence, 2, 128],
        )
        if output["storage_sha256"] != block_residual["storage_sha256"]:
            raise RuntimeError(
                "layer-one Engram output does not feed layer-one block input"
            )
    ...
    return {
        "schema_version": 1,
        "scope": (
            "layer-one source Engram hash, embedding, FP8 WKV projection, and residual "
            "gate into the layer-one block entry; not earlier layer residual production, "
        ...
        json.dumps(
            {
                ...
                "status": "source_layer_one_engram_fixture",
            },
            sort_keys=True,
        )
    )
